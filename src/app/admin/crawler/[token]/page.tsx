/**
 * /admin/crawler/[token] — read-only crawler analytics dashboard.
 *
 * Token-protected: the URL token must match process.env.ADMIN_TOKEN.
 * If it doesn't (or ADMIN_TOKEN is unset), the page 404s.
 *
 * Server Component — runs on Vercel function (not edge) so it can use the
 * Supabase service-role key and aggregate freely.
 */
...
export const metadata: Metadata = {
  title: "Crawler Analytics — Practiq Admin",
...
export default async function CrawlerDashboard({ params }: PageProps) {
  ...
  // Pull the last 30 days of hits in one shot, then aggregate in JS.
  // (Supabase JS client doesn't expose GROUP BY directly without RPC.)
  ...
  if (error) {
    const msg = error.message.includes("relation") || error.message.includes("does not exist")
      ? "The crawler_hits table doesn't exist yet. Apply supabase/migrations/20260414100000_crawler_hits.sql in the Supabase SQL editor."
  ...
  const catMap = new Map<
    string,
    { hits: number; bots: Set<string>; paths: Set<string> }
  >();
  for (const r of allRows) {
    const existing = catMap.get(r.bot_category) ?? {
      hits: 0,
      bots: new Set<string>(),
      paths: new Set<string>(),
    };
    existing.hits += 1;
    existing.bots.add(r.bot_name);
    existing.paths.add(r.path);
    catMap.set(r.bot_category, existing);
  }
  const byCategory: CategoryRow[] = Array.from(catMap.entries())
    .map(([bot_category, v]) => ({
      bot_category,
      hits: v.hits,
      unique_bots: v.bots.size,
      unique_paths: v.paths.size,
    }))
  ...
  const botMap = new Map<
    string,
    { category: string; hits: number; paths: Set<string>; last_seen: string }
  >();
  for (const r of allRows) {
    const existing = botMap.get(r.bot_name) ?? {
    ...
    existing.hits += 1;
    existing.paths.add(r.path);
    if (r.hit_at > existing.last_seen) existing.last_seen = r.hit_at;
    botMap.set(r.bot_name, existing);
  }
      ...
      <div className="max-w-6xl mx-auto py-12 px-6">
        ...
        <Section title="By category">
          {byCategory.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {byCategory.map((c) => {
                const meta = CATEGORY_LABELS[c.bot_category] ?? {
                  label: c.bot_category,
                  color: "text-zinc-400",
                };
                return (
                  <div
                  ...
                  >
                    <div
                      className={`text-[10px] font-bold uppercase tracking-widest mb-2 ${meta.color}`}
                    >
                      {meta.label}
                    </div>
                    ...
                    <div className="text-xs text-zinc-500">
                      {c.unique_bots} bot{c.unique_bots !== 1 ? "s" : ""} ·{" "}
                      {c.unique_paths.toLocaleString()} pages
                    </div>
                  </div>
                );
              })}
        ...
        <Section title="Top bots (last 30 days)">
          <Table
            ...
            rows={topBots.map((b) => [
              <span key="b" className="font-mono text-zinc-200">
                {b.bot_name}
              </span>,
              <CategoryBadge key="c" category={b.bot_category} />,
              <span key="h" className="text-zinc-200">
                {b.hits.toLocaleString()}
              </span>,
              <span key="p" className="text-zinc-400">
                {b.unique_paths.toLocaleString()}
              </span>,
              <span key="l" className="text-xs text-zinc-500 font-mono">
                {formatRelative(b.last_seen)}
              </span>,
            ])}
          />
        </Section>
        ...
        <Section title="Most-crawled pages (last 30 days)">
          <Table
            ...
            rows={topPaths.map((p) => [
              ...
              <span key="b" className="text-zinc-400">
                {p.unique_bots}
              </span>,
            ])}
          />
          ...
          <Table
            ...
            rows={recent.map((r) => [
              <span key="t" className="text-xs text-zinc-500 font-mono">
                {formatRelative(r.hit_at)}
              </span>,
              <span key="b" className="font-mono text-zinc-300">
                {r.bot_name}
              </span>,
              <span
                key="p"
        ...
        <footer className="mt-12 text-xs text-zinc-600 text-center">
          ...
          <br />
          Source: Supabase <code className="text-zinc-400">crawler_hits</code> table populated via Edge middleware.
        </footer>
      </div>
    </DashboardShell>
  );
        ...
        <thead className="bg-zinc-900/50 border-b border-zinc-800">
          <tr>
            {headers.map((h) => (
              <th
                key={h}
                className="text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-zinc-500"
              >
                {h}
              </th>
            ))}
...
function CategoryBadge({ category }: { category: string }) {
  ...
  return (
    <span
      className={`text-[10px] px-2 py-1 rounded-md bg-zinc-800 font-bold uppercase tracking-widest ${meta.color}`}
    >
      {meta.label}
    </span>
  );
